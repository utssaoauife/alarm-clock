...
void cont_data_read(char data[], unsigned char start, unsigned char len, char end_char){
    unsigned char i;
    for(i = 0; i < len; i++){
        data[i] = data_read(start + i);
        delay();
    }
    data[i] = end_char;
}
...
char set_time(char arr[], char i){
    RC5 = 0;
    for(char j = 0; j < 2 && RC1; j++){
        for(char k = 0; k < 40 && RC1; k++){
            disp_dig(1, ' ');
            disp_dig(2, 5);
            disp_dig(3, 'E');
            disp_dig(4, 'T');
    ...
    if(RC1){
        ...
        while(1){
            ...
            if(RC2){
                arr[i]++;
                if(i && (arr[i] == 60)) arr[i] = 0;
                if(!i && (arr[i] == 24)) arr[i] = 0;
                for(char j = 0; j < 25; j++){
                    disp(arr[0], arr[1]);
                    real_time_counter();
                }
            ...
            if(RC3){
                if(arr[i] > 0) arr[i]--;
                else if(i && (arr[i] == 0)) arr[i] = 59;
                else if(!i && (arr[i] == 0)) arr[i] = 23;
        ...
        for(alarm_no = 0; alarm_no < 4; alarm_no++){
            ...
            if(((al_state & (1 << alarm_no)) && (time_sum == alarms[alarm_no])) || ((snz_state & (1 << alarm_no)) && (time_sum == snz[alarm_no]))){
                ...
                for(char i = 0; i < 15; i++){
                    ...
                    for(char j = 0; j < 2 && !(RC1 || RC0 || RC2); j++) buzz(alarm_h, alarm_m);
                    if(RC1 || RC0 || RC2) break;

                    for(char j = 0; j < 10 && !(RC1 || RC0 || RC2); j++){
                    ...
                    for(char j = 0; j < 50 && !(RC1 || RC0 || RC2); j++){
                        disp(alarm_h, alarm_m);
                        real_time_counter();
                    }
                    if(RC1 || RC0 || RC2) break;
